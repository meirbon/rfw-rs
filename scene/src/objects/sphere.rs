...
impl Intersect for Sphere {
    ...
    fn intersect4(&self, packet: &mut RayPacket4, t_min: &[f32; 4]) -> Option<[PrimID; 4]> {
        ...
        let a_x: glam::Vec4 = direction_x * direction_x;
        let a_y: glam::Vec4 = direction_y * direction_y;
        let a_z: glam::Vec4 = direction_z * direction_z;
        let a: glam::Vec4 = a_x + a_y + a_z;

        let r_pos_x: glam::Vec4 = origin_x - glam::Vec4::from([self.pos[0]; 4]);
        let r_pos_y: glam::Vec4 = origin_y - glam::Vec4::from([self.pos[1]; 4]);
        let r_pos_z: glam::Vec4 = origin_z - glam::Vec4::from([self.pos[2]; 4]);

        let b_x: glam::Vec4 = direction_x * 2.0 * r_pos_x;
        let b_y: glam::Vec4 = direction_y * 2.0 * r_pos_y;
        let b_z: glam::Vec4 = direction_z * 2.0 * r_pos_z;
        let b: glam::Vec4 = b_x + b_y + b_z;

        let r_pos2_x: glam::Vec4 = r_pos_x * r_pos_x;
        let r_pos2_y: glam::Vec4 = r_pos_y * r_pos_y;
        let r_pos2_z: glam::Vec4 = r_pos_z * r_pos_z;
        let r_pos2: glam::Vec4 = r_pos2_x + r_pos2_y + r_pos2_z;

        let radius: glam::Vec4 = glam::Vec4::from([self.radius2; 4]);
        let c: glam::Vec4 = r_pos2 - radius;
        let d: glam::Vec4 = b * b - 4.0 * a * c;

        let t_min = glam::Vec4::from(*t_min);

        let mask = d.cmpge(glam::Vec4::zero());
        // No hits
        if mask.bitmask() == 0 {
            return None;
        }
