...
impl Node {
    pub fn with_parent(mut self, id: u32) -> Self {
        self.parent = id;
        self
    }
    ...
    pub(crate) fn update(&mut self, matrix: Mat4) {
        ...
        for object in &mut self.objects {
            match object {
                NodeChild::Instance2D(i) => i.set_matrix(matrix),
                NodeChild::Instance3D(i) => i.set_matrix(matrix),
                NodeChild::Custom(_) => {}
            }
        }
    }
    ...
    pub(crate) fn remove_child(&mut self, child: u32) {
        assert!(self.children.remove(&child), "Could not remove child {}", child);
    }
    ...
    pub fn get_objects_mut(&mut self) -> &mut [NodeChild] {
...
impl Drop for Node {
    fn drop(&mut self) {
        for object in self.objects.iter_mut() {
            unsafe {
                match object {
                    NodeChild::Instance2D(i) => i.clone_handle().make_invalid(),
                    NodeChild::Instance3D(i) => i.clone_handle().make_invalid(),
                    NodeChild::Custom(_) => {}
                }
            }
        }
